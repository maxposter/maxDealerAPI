...
class maxThumbnail extends maxOption
{
  ...
  protected function getDefaultOptions()
  {
    return array(
      ...
      'source_photo_url' => 'http://www.maxposter.ru/photo/%s/%s/original/%s',
   ...
   */
  protected function getSourcePhotoUrl($_dealerId, $_autoId, $_imageId)
  {
    return sprintf($this->getRequiredOption('source_photo_url'), $_dealerId, $_autoId, $_imageId);
  }
   ...
   * Загрузка фото
   *
   * @param string $_path   Путь к фото (URL либо локальный путь)
   * @return  Фото
   */
...
<p>The requested URL '.htmlspecialchars($_SERVER['REQUEST_URI']).' was not found on this server.</p>
  ...
  {
    $urlPattern = '\/([0-9]*)\/('.implode('|', array_keys($this->getRequiredOption('allowed_photo_sizes'))).')\/([0-9a-z]*.('.implode('|', $this->getRequiredOption('photo_extensions')).'))$';
  ...
  public function getPhoto($_url)
  {
    ...
    catch (Exception $e)     // При возникновении любого сбоя возвращаем посетителю 404 страницу
    {
      $this->show404page();
    }
  }
